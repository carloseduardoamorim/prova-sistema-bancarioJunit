...
public class Movimentacao implements Cadastro {
    ...
    public void setTipo(char tipo){
        if(tipo != 'C' && tipo != 'D'){
            throw new IllegalArgumentException("Tipo deve ser somente 'C' para "
    ...
    public void setValor(double valor) {
        ...
        if (tipo == 'D') {
            if (valor > conta.getSaldoTotal()) {
                throw new IllegalArgumentException("O valor da movimentação não "
                        + "pode ser maior que o saldo total");
            }
        }
    ...
    public boolean isConfirmada() {
        if(confirmada == false){
            throw new IllegalArgumentException("Valor padrão deve ser verdadeiro");
        }
        return confirmada;
    }
